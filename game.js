...
function setupHiDPI() {
  ...
  // Give the canvas enough REAL pixels for crisp rendering
  ...
  // Map logical coordinates (276x414) to device pixels
  sctx.setTransform(internalW / W(), 0, 0, internalH / H(), 0, 0);
  sctx.imageSmoothingEnabled = true;          // smoother scaling
  // Optional:
  // sctx.imageSmoothingQuality = "high";
}

// Run now and whenever the window resizes
setupHiDPI();
...
function updateLeaderboard(name, score) {
  // Allow duplicate scores (ties) and multiple players with the same score.
  // Each run can create a new entry. We keep only the top 5 after sorting.
  if (!(score > 0)) return;
...
function drawLeaderboard(ctx, scrn) {
  const lb = loadLeaderboard();
  ctx.save();
  ctx.textAlign = "right";
...
scrn.onkeydown = function keyDown(e) {
  ...
  // Handle name typing in getReady
  if (state.curr === state.getReady) {
    const k = e.key;
    if (k === 'Backspace') {
      nameEntry.text = nameEntry.text.slice(0, -1);
      return;
    }
    if (k === 'Enter') {
    ...
    if (k && k.length === 1) {
      if (nameEntry.text.length < 16) nameEntry.text += k;
      return;
    }
    // Ignore other keys until name entered
    return;
  }
  switch (state.curr) {
      case state.getReady:
...
const bg = {
  ...
  y: 0,
  draw: function () {
    y = parseFloat(H() - this.sprite.height);
...
const UI = {
  ...
  draw: function () {
    switch (state.curr) {
      case state.getReady:
        ...
        (function(){
          ...
          sctx.lineWidth = 3;
          const cx = W() / 2;
          const cy = (parseFloat(H() - UI.getReady.sprite.height) / 2) + UI.getReady.sprite.height + 10;
    ...
    this.drawScore();
    try { drawLeaderboard(sctx, scrn); } catch (e) {}
  },
  drawScore: function () {
    sctx.fillStyle = "#FFFFFF";
    sctx.strokeStyle = "#000000";
    switch (state.curr) {
      ...
      case state.gameOver:
        ...
        // Update leaderboard once per game over draw
